...
SELECT  persona.apellido1 , persona.apellido2 , persona.nombre , departamento.nombre as dept
FROM persona p
left JOIN profesor
ON p.id = profesor.id_profesor
inner join dept
ON profesor.id_dept = departamento.id
...
SELECT persona.nif, persona.nombre, persona.apellido1, asignatura.nombre as asignatura, curso_escolar.anyo_inicio , curso_escolar.anyo_fin 
FROM asignatura 
left join alumno_se_matricula_asignatura as al
on asignatura.id = al.id_asignatura
...
LEFT JOIN dpt
ON profesor.id_dpt = departamento.id
ORDER BY dpt.nombre,  persona.apellido1, persona.apellido2, persona.nombre ASC;
...
ON persona.id = profesor.id_profesor
LEFT JOIN dpt
ON profesor.id_departamento = dpt.id
WHERE dpt.nombre IS NULL;
...
ON persona.id =profesor.id_profesor  
LEFT JOIN asignatura
ON profesor.id_departamento = asig.id_profesor
WHERE persona.tipo = 'profesor'
AND asig.nombre IS NULL
...
FROM asignatura
LEFT JOIN profesor
ON asig.id_profesor = profesor.id_departamento    
LEFT JOIN persona
ON profesor.id_profesor = persona.id 
WHERE persona.apellido1 IS NULL;
...
SELECT departamento.nombre AS dpt, persona.apellido1, persona.nombre  
FROM departamento
LEFT JOIN profesor
ON dept.id = profesor.id_dpt
LEFT JOIN persona
ON profesor.id_profesor =persona.id;
